...
public class SegmentHome {
	...
	public Segment merge(Segment detachedInstance) {
		...
		try {
			Segment result = (Segment) sessionFactory.getCurrentSession()
	...
	public Segment findById(java.lang.Integer id) {
		...
		try {
			Segment instance = (Segment) sessionFactory.getCurrentSession()
					.get("posra.dataaccess.Segment", id);
			if (instance == null) {
	...
	public List findByExample(Segment instance) {
		log.debug("finding Segment instance by example");
		try {
			List results = sessionFactory.getCurrentSession()
					.createCriteria("posra.dataaccess.Segment")
